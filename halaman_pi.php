<?php
    ...
    $query2 = query("SELECT * FROM file WHERE id_author='$id_author'");
    if (isset($_POST["cari"])) {
        $query = cari2($_POST["keyword"]);
    }
    if (isset($_POST["approve_button1"])) {
    ...
    if (isset($_POST["approve_button2"])) {
        $query2 = approved($id_author);
        {
            echo "
                <script>
                    alert('Data berhasil diapprove!');
         ...
         <nav class="navbar navbar-expand-lg navbar-transparent navbar-dark">
            ...
            <div class="row mt-4">
                ...
                <div class="col text-end" data-aos="fade-left">
                    <form class="d-flex" method="post">
                        <input class="form-control" id="approve_button1" type="hidden" name="keyword">
                        <button class="btn btn-success" type="submit" name="approve_button1">Approve Tahap 1</button>
                    </form>
                </div>
                <div class="col text-end" data-aos="fade-left">
                    <form class="d-flex" method="post">
                        <input class="form-control" id="approve_button2" type="hidden" name="keyword">
                        <button class="btn btn-success" type="submit" name="approve_button2">Approve Tahap 2</button>
                    </form>
                </div>
                <div class="col text-end" data-aos="fade-left">
                    <a href="tambah_nilai.php" class="btn btn-success btn" tabindex="-1" role="button" aria-disabled="true"><i class="bi-box-arrow-in-right"></i>&nbsp;&nbsp;Input Nilai</a>
                </div>
            </div>
            <br>
                ...
                <table class="table table-secondary table-hover" border="1" cellpadding="10" cellspacing="0" data-aos="zoom-in" data-aos-duration="1000">
                    <thead>
                        <tr  class="d-flex text-center">
                            ...
                            <th width="175px">Dosen Pembimbing</th>
                            <th width="130px">Surat Permohonan Fakultas</th>
                            <th width="130px">Surat Balasan Instansi</th>
                            <th width="140px">Proposal</th>
                            <th width="111px">Tahapan</th>
                            <th width="111px">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                      <?php foreach($query as $row) :?>
                        <tr class="d-flex text-center">
                            ...
                            <td width="175px"><?= $row["Judul"]; ?></td>
                            <td width="175px"><?= $row["dospem"]; ?></td>
                            <td width="130px"><?= $row["surfak"]; ?></td>
                            <td width="130px"><?= $row["surbal"]; ?></td>
                            <td width="140px"><?= $row["proposal"]; ?></td>
                            <td width="111px"><?= $row["tahap"]; ?></td>
                            <td width="111px"><?php if($row["is_approved"] == true) {
                                echo "Approved";
                            } else {
                                echo "Pending";
                            }; ?></td>
            ...
            <div class="table-responsive">
                <table class="table table-secondary table-hover" border="1" cellpadding="10" cellspacing="0" data-aos="zoom-in" data-aos-duration="1000">
                    <thead>
                    <tr class="d-flex text-center">
                            <th width="155px">Jadwal Sidang</th>
                            <th width="135px">Waktu Sidang</th>
                            <th width="210px">Dosen Penguji</th>
                            <th width="170px">Laporan</th>
                            <th width="170px">Nilai PI Instansi</th>
                            <th width="170px">Lembar Pengesahan</th>
                            <th width="155px">Tahapan</th>
                    ...
                    <tbody>
                      <?php foreach($query2 as $row) :?>
                        <tr class="d-flex text-center">
                            <td width="155px"><?= $row["jadwal"]; ?></td>
                            <td width="135px"><?= $row["pukul"]; ?></td>
                            <td width="210px"><?= $row["uji"]; ?></td>
                            <td width="170px"><?= $row["laporan"]; ?></td>
                            <td width="170px"><?= $row["nilai"]; ?></td>
                            <td width="170px"><?= $row["pengesahan"]; ?></td>
                            <td width="155px"><?= $row["tahapan"]; ?></td>
                            <td width="125px"><?php if($row["is_approved1"] == true) {
                                echo "Approved";
                            } else {
                                echo "Approved";
                            }; ?></td>
